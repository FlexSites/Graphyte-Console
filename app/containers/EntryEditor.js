          ...
          <div style={{ width: '100%' }}>
            <Toolbar style={{ backgroundColor: get(this, 'context.muiTheme.palette.accent1Color') }}>
              <ToolbarGroup firstChild={true}>
                <ToolbarTitle text={this.props.entry.name} style={{ fontWeight: '100', marginLeft: '10px' }} />
              </ToolbarGroup>
              <ToolbarGroup lastChild={true}>
                <div style={{ alignSelf: 'center' }}>
                  <FlatButton icon={<Definition />} label="Definition" primary={!this.isActive()} onTouchTap={() => this.props.push(`/edit/${this.props.entry.id}`)}/>
                  <FlatButton icon={<Cloud />} label="Resolver" primary={!this.isActive('resolve')} onTouchTap={() => this.props.push(`/edit/${this.props.entry.id}/resolve`)}/>
                  <FlatButton icon={<FlipToBack />} label="Mock" primary={!this.isActive('mock')} onTouchTap={() => this.props.push(`/edit/${this.props.entry.id}/mock`)}/>
                </div>
              </ToolbarGroup>
            </Toolbar>
            {this.props.children}
